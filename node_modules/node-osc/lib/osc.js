...
var Client = function (port, bindPort, host, debug) {
    ...
    this._sock.on('message', function (msg, rinfo) {
        ...
        //console.log('decoded:', decoded, rinfo)
...
Client.prototype = {
    ...
    send: function () {
        ...
        while(data = args.shift()) {
          if ('undefined' !== typeof data) msg.append(data);
        }
        this.msg(msg);
    },
    on: function(address, callback) {
        ...
        this._callbacks.push({'address': address, 'callback': callback});
    },
    removeListener: function(address) {
        for (var c=0; c<this._callbacks.length; c++) {
            // if they're matching strings or matching regex
            if (this._callbacks[c] && address_matches(address, this._callbacks[c].address)) {
...
// check regex and string matches
var address_matches = function(address, test) {
    return (typeof(test) == 'string' || address == test?
       // string to string or regex to regex
       address == test:
       // string to regex
       address.match(test));
};
...
var Server = function(port, host) {
    ...
    this.on = function(address, callback) {
        // if not regex do some replacement
        if (typeof(address) == 'string') {
            address = '/' + address.replace('/', '');
        }
